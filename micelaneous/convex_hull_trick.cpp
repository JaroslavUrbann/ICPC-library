...
struct Line{ll k=0,b=LLONG_MAX;};
Line operator-(Line a,Line b){return {a.k-b.k,a.b-b.b};}

// positive if b is to the left of a
ll cross(Line a,Line b){return a.k*b.b-a.b*b.k;}
...
struct CHT{
	// vecs are sorted by smallest angle from the x axis up ((1,0),(1,1)...)
	...
	void addline(Line l){
		if(hull.size()){
			...
			if(auto [k,b]=hull.back();l.k>k||(l.k==k&&l.b<b)){
				...
				if(hull.size())vecs.push_back({-(hull.back().b-l.b),hull.back().k-l.k});
				hull.push_back(l);
				...
				if(hull.size())vecs.push_front({-(hull[0].b-l.b),hull[0].k-l.k});
				hull.push_front(l);
	...
	ll get(ll x){
		...
		auto it=lower_bound(vecs.begin(),vecs.end(),q,[&](Line a,Line b){
			return cross(a,b)>0;
		});
